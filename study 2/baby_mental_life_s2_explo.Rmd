...
If we look back at Study 1, do we see similar effects by parent status?
...
scores_S1 <- efa_S1$scores %>%
  ...
  left_join(demo_S1 %>% distinct(ResponseId, Parent) %>%
              ...
              mutate(subid = as.character(subid))) %>%
  mutate(factor_names = recode_factor(factor,
                                      "MR4" = "Negative emotions (S1)",
                                      "MR1" = "Cognition & control (S1)",
                                      "MR2" = "Bodily sensations (S1)",
                                      "MR3" = "Positive/social emotions (S1)"),
         factor = factor(factor))
```
...
scores_S1 %>%
  ...
  geom_line(data = d_parent_boot_s1, 
            position = position_dodge(width = 5), lty = 2,
            aes(y = mean, group = parent)) +
...
(I won't bother re-plotting with other treatments of age, since there are only 3 target ages and it's easy enough to see them here.)
